...
//
//            if(!"1".equals(status)){
//                Log.d("the order is not success from chongchong server. statusCode:%s;orderID:%s", statusCode, orderID);
...
//            }
            ...
            if(!isSignOK(channel)){
                Log.d("the sign is not valid. sign:%s;orderID:%s", sign, kpordernum);
    ...
    private boolean isSignOK(UChannel channel){
        ...
        Log.d("kaopu check sign orig str:");
        Log.d(sb.toString());
